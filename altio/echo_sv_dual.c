...
main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        if (nready < 0) {            
			...
			/* The code which calls write (or other blocking operations) has to be aware of EINTR. 
             * If a signal occurs during a blocking operation, then the operation will either 
             * (a) return partial completion, or 
             * (b) return failure, do nothing, and set errno to EINTR.
